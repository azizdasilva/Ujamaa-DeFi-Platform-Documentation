...
contract NAVCalculator {
     ...
     *
     * @param totalPoolValue Total value of pool assets (in Ondo EUROD (EUROD), 18 decimals)
     ...
     *
     * Formula: uLP Amount = Ondo EUROD (EUROD) Deposit / NAV Per Share
     *
     * @param eurcAmount Ondo EUROD (EUROD) deposit amount (18 decimals)
    ...
    function calculateULPMintAmount(
        ...
        // uLP = (Ondo EUROD (EUROD) * PRECISION) / NAV
    ...
    /**
     * @dev Calculate Ondo EUROD (EUROD) to receive on redemption
     *
     * Formula: Ondo EUROD (EUROD) = uLP Shares * NAV Per Share
     *
     * @param ulpAmount uLP tokens to redeem (18 decimals)
     * @param navPerShare Current NAV per share (18 decimals)
     * @return eurcAmount Ondo EUROD (EUROD) to receive (18 decimals)
     */
    function calculateEURCRedemption(
        uint256 ulpAmount,
        uint256 navPerShare
    ) external pure returns (uint256 eurcAmount) {
        // Ondo EUROD (EUROD) = (uLP * NAV) / PRECISION
...
def calculate_nav_per_share(
    ...
    Args:
        total_pool_value: Total pool value in Ondo EUROD (EUROD) (18 decimals)
...
def calculate_ulp_mint_amount(
    ...
    """
    Calculate uLP tokens to mint on Ondo EUROD (EUROD) deposit.

    Args:
        eurc_amount: Ondo EUROD (EUROD) deposit (18 decimals)
    ...
    Ondo EUROD (EUROD) = Decimal(eurc_amount)
...
def calculate_eurc_redemption(
    ...
    """
    Calculate Ondo EUROD (EUROD) to receive on uLP redemption.
    ...
    Returns:
        Ondo EUROD (EUROD) to receive (18 decimals)
    """
    ...
    return int(Ondo EUROD (EUROD).quantize(Decimal('1'), rounding=ROUND_DOWN))
```
...
```
Initial State:
  - Total Pool Value: 1,000,000 Ondo EUROD (EUROD) (€1M)
  - Total uLP Shares: 1,000,000 (1M shares)
  - NAV Per Share: 1.000000000000000000 Ondo EUROD (EUROD)

After Yield Accrual (5% APY, 1 year):
  - Total Pool Value: 1,050,000 Ondo EUROD (EUROD) (€1.05M)
  - Total uLP Shares: 1,000,000 (unchanged)
  - NAV Per Share: 1.050000000000000000 Ondo EUROD (EUROD) (+5%)

Investor Redemption:
  - Investor holds: 10,000 uLP shares
  - Ondo EUROD (EUROD) Received: 10,000 × 1.05 = 10,500 Ondo EUROD (EUROD)
 ...
 *
 * @param totalYield Total yield to distribute (Ondo EUROD (EUROD), 18 decimals)
 * @param totalPoolValue Current total pool value (Ondo EUROD (EUROD), 18 decimals)
 ...
 * @return investorYield Investor's share of yield (Ondo EUROD (EUROD), 18 decimals)
 */
...
struct Guarantee {
    ...
    uint256 merchandiseValue;   // Value in EUROD (18 decimals)
...
- `value` - Merchandise value in EUROD (18 decimals)
...
struct Certificate {
    ...
    uint256 value;            // Value in EUROD (18 decimals)
    uint256 quantity;
...
- `value` - Value in EUROD (18 decimals)
...
contract DiversificationChecker {
     ...
     *
     * @param poolValue Total pool value (Ondo EUROD (EUROD), 18 decimals)
     * @param currentIndustrialExposure Current exposure to industrial (Ondo EUROD (EUROD), 18 decimals)
     * @param currentAssetClassExposure Current exposure to asset class (Ondo EUROD (EUROD), 18 decimals)
     * @param newFinancingAmount New financing amount (Ondo EUROD (EUROD), 18 decimals)
