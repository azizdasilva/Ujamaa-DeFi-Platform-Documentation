...
1. **Ujamaa Pool Token (uLP) (Yield-Bearing)**
   ...
   - Redeem uLP → receive Ondo EUROD (EUROD) + yield
...
| **Ondo EUROD (EUROD)** | Euro Coin; euro-backed stablecoin (1 Ondo EUROD (EUROD) = €1.00). Issued by Ondo Finance (MiCA-compliant, EU-regulated) |
...
25. **Ondo Finance Ondo EUROD (EUROD) Documentation** — Euro Coin Stablecoin (https://www.Ondo Finance.com/en/Ondo EUROD (EUROD))
...
16. **Ujamaa Pool Token (uLP) Minting & Redemption** — Create yield-bearing uLP tokens representing pool ownership; redeem uLP for Ondo EUROD (EUROD) + accrued yield using value-accrual model (balance constant, NAV increases)
...
- Request **redemption** (uLP → Ondo EUROD (EUROD) + yield) 🆕
...
**Strategy:** Ondo EUROD (EUROD)/EURR Primary Stablecoins with 2% Transaction Fee Margin
...
```
Investor (EUR) → Ondo EUROD (EUROD) (1:1 parity) → Pool (Ondo EUROD (EUROD)-denominated)
                                          ↓
                                    Industrial (FCFA)
                                          ↓
                                    Repayment (Ondo EUROD (EUROD))
                                          ↓
                                    Investor (Ondo EUROD (EUROD) + yield)

FX Risk Absorption:
- Entry: EUR → Ondo EUROD (EUROD) (0% slippage, 1:1 parity)
- Deployment: Ondo EUROD (EUROD) → FCFA (2% fee margin covers conversion)
- Repayment: FCFA → Ondo EUROD (EUROD) (2% fee margin covers conversion)
- Exit: Ondo EUROD (EUROD) → EUR (0% slippage, 1:1 parity)
...
| **Investment (Ondo EUROD (EUROD) → FCFA)** | 2% | Covers EUR/FCFA conversion risk |
| **Repayment (FCFA → Ondo EUROD (EUROD))** | 2% | Covers FCFA/EUR conversion risk |
| **Redemption (Ondo EUROD (EUROD) → EUR)** | 0% | No FX risk (1:1 parity) |
| **Total FX Coverage** | **4% round-trip** | **Fully hedged** |

**Ondo EUROD (EUROD) vs. USDC/USDT Comparison:**

| Stablecoin | Peg | FCFA Correlation | FX Risk (vs. FCFA) | Primary Use |
|------------|-----|------------------|-------------------|-------------|
| **Ondo EUROD (EUROD)** ✅ | EUR (1:1) | **Fixed (1 EUR = 655.957 XOF)** | **None** | **Primary (African markets)** |
...
**Why Ondo EUROD (EUROD)/EURR (Not USDC/USDT)?**

1. **EUR/FCFA Fixed Parity:** 1 EUR = 655.957 XOF (fixed since 1994, BCEAO guarantee)
2. **Zero FX Risk:** Ondo EUROD (EUROD) maintains 1:1 parity with FCFA (no fluctuation)
3. **Historical Stability:** EUR/FCFA rate unchanged for 30+ years
4. **African Market Fit:** Ondo EUROD (EUROD) aligns with UEMOA monetary zone (8 countries, 160M people)
5. **Platform Risk Absorption:** 2% fee margin covers any residual FX exposure

**Risk Mitigation Hierarchy:**

```
Level 1: Ondo EUROD (EUROD)/EURR Primary (Zero FX Risk)
...
|-------------|----------------|
| **BCEAO Compliance** | Ondo EUROD (EUROD)/FCFA parity maintained (UEMOA monetary zone) |
| **Ondo Finance Ondo EUROD (EUROD) Licensing** | Ondo EUROD (EUROD) issued by Ondo Finance (EU MiCA compliant) |
...
| FR ID | Requirement (Atomic, Testable) | User Story | Acceptance Criteria | Test Condition |
|-------|-------------------------------|------------|---------------------|----------------|
| **FR-068** | System SHALL mint Ujamaa Pool Token (uLP) as yield-bearing ERC-3643 fungible token | US-10.1 | AC-10.1.1, AC-10.1.2 | TC-068: Deposit EUROD → uLP minted at current NAV |
...
| **FR-071** | System SHALL allow uLP redemption for EUROD at current NAV (instant, no lock-up) | US-10.2 | AC-10.2.1, AC-10.2.3 | TC-071: Redeem uLP → EUROD transferred, tokens burned |
...
**As an** Institutional Investor,
**I want** to redeem uLP tokens for Ondo EUROD (EUROD),
**So that** I can exit my investment and receive principal + accrued yield.

**Acceptance Criteria:**
- [ ] Redeem uLP → receive Ondo EUROD (EUROD) at current NAV
...
- `POST /v1/otex/orders` — Convert fiat → stablecoin (EUR → Ondo EUROD (EUROD))
...
class FireblocksService:
    ...
    def get_balance(self, vault_id: str) -> int:
        """Get vault balance in Ondo EUROD (EUROD)."""
```
...
- Ondo EUROD (EUROD) (redeemed by institutional investors)
...
**Inter-Layer Dependencies:**
- Depends on **Tokenization Layer** for Ondo EUROD (EUROD) token contract
...
- Depends on **Payments Layer** for Ondo EUROD (EUROD) distribution
